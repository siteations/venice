  	      ...
  	      {details.map((d, i)=>{
  	      		...
  	      		<g key={`detail${i}`}>
	    	   				<image xlinkHref={d.srcThumb} x={d.x} y={d.y} width={d.width} height={d.height} clipPath={d.clip} />
					    		...
					    		<rect stroke="#ffffff"
                  className="circHL"
                  x={clipDetails[i].x} y={clipDetails[i].y} rx={clipDetails[i].rx} ry={clipDetails[i].ry} width={clipDetails[i].width} height={clipDetails[i].height} id={d.id} onTouchTap={e=> action(e)}/>
